...
const DropDown = ({ toggle, isOpen }) => {
  ...
  return (
    <div className={isOpen ? 'sticky top-0 h-screen  z-50 grid grid-cols-1 gap-5 bg-white text-center space-y-3 py-5' : 'hidden'}
    >
           <div className='absolute cursor-pointer right-5 top-5' onClick={toggle}>
          <FaTimes size={25} className=' text-slate-900 mr-2'/>
          </div>
      <div    div className='flex flex-col items-start  space-y-5 pt-10 pl-5 h-full'>
                    ...
                    {serviceCategoryData.isFetched ? (
                      ...
                      {serviceCategoryData?.data?.data?.data?.map((item) => (
                          <div className="flex flex-col items-start space-y-2">
                            <Link onClick={toggle} className="hover:opacity-80" to={`/services/${item.id}`}>
